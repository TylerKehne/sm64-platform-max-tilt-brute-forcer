...
int main() {
    ...
    for (float startX = minX; startX <= maxX; startX += 0.25f) {
        ...
        for (float startZ = minZ; startZ <= maxZ; startZ += 0.25f) {
            ...
            // Vec3f startNormal = { -0.2221239423751831f, 0.8842829847335815f,
            // 0.3479344165325165f }; Normal from triple bully blueprint
            // assuming a 4f 10k route
            Vec3f startNormal = {
                -0.2221239423751831f, 0.8842829847335815f, 0.3479344165325165f};
            ...
            if (path.BestFrame != -1 && path.BestBoundaryDist > 0.0f) {
                fprintf(
                    file,
                    "%.50g,%.50g,%.50g,%.50g,%.50g,%.50g,%.50g,%.50g,%.50g,%.50g,%.50g,%.50g,%d,%.50g\n",
                    path.StartNormal[0], path.StartNormal[1],
                    path.StartNormal[2], path.StartMarioPos[0],
                    path.StartMarioPos[1], path.StartMarioPos[2],
                    path.BestNormal[0], path.BestNormal[1], path.BestNormal[2],
                    path.BestMarioPos[0], path.BestMarioPos[1],
                    path.BestMarioPos[2], path.BestFrame,
